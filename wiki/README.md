# PhizMusic — Physics-First Music Theory Wiki

Music theory rebuilt from physics, for minds that think in equations.

PhizMusic is a foundational music theory framework that derives every concept from physics, mathematics, and neuroscience. Where conventional Western music theory relies on 18th-century terminology and notation conventions that obscure the underlying physics, PhizMusic uses physics-literal vocabulary, transparent mathematical relationships, and psychoacoustic evidence to explain how music works and why it sounds the way it does.

Every culture's music system is a different engineering solution to the same mathematical constraints. PhizMusic provides the shared physics vocabulary to understand them all.

## Target Audience

This wiki is designed for the scientifically-minded learner — someone with a solid understanding of physics, math, and biology who appreciates multicultural music but finds conventional Western music theory incomprehensible. If you've ever wondered why a "fifth" is called a "fifth" when it's actually a 3:2 frequency ratio, this framework is for you.

## Design Principles

- **Physics-literal vocabulary** — every term means what it says. A step-distance of 7 is always 7 chromatic steps. A frequency ratio of 3:2 is always 3:2.
- **Psychoacoustics woven throughout** — perceptual neuroscience isn't a separate chapter; it's integrated into every concept, explaining WHY things sound the way they do.
- **Dual-level writing** — rigorous college-level explanations with `> 🎯 **Simple version**` blocks for quick intuitive understanding.
- **Translation tables** — every relevant page maps PhizMusic terms to Western and other cultural equivalents. 100% internal elegance; translation tables handle external communication.
- **Cultural respect** — different tuning systems and musical traditions are "different engineering solutions to the same mathematical constraints," never "inferior approximations."
- **Atomic concept pages** — one concept per page, interconnected by links. Enter anywhere, follow your curiosity.

## How to Read This Wiki

There is no required reading order. Each page is self-contained, with prerequisites listed in its front matter. Start with whatever interests you and follow the links.

**Suggested starting points:**
- New to sound physics? → [Sound Waves](sound-waves.md)
- Want the PhizMusic naming system? → [Naming System](naming-system.md)
- Curious about why certain note combinations sound good? → [Consonance & Dissonance](consonance-dissonance.md)
- Need to translate PhizMusic ↔ Western terms? → [Translation Tables](translation-tables.md)

## Concept Map

### Tier 1 — Physics Primitives

The physical foundations that everything else builds on.

| Page | Description |
|------|-------------|
| [Sound Waves](sound-waves.md) | Pressure waves, frequency, amplitude, waveform vs. spectrum |
| [The Ear & Cochlea](ear-cochlea.md) | Biological frequency analyzer, basilar membrane, critical bandwidth |
| [Fourier Analysis](fourier-analysis.md) | Decomposing complex sounds into sine wave components |

### Tier 1 — Music Primitives

The core musical concepts derived from physics.

| Page | Description |
|------|-------------|
| [Harmonic Series](harmonic-series.md) | The natural overtone sequence: f, 2f, 3f, 4f... |
| [Frequency Ratios](frequency-ratios.md) | Comparing pitches by ratio, the cent system |
| [Naming System](naming-system.md) | Step-numbers (0-11) + Dodeka syllables (Do, Ka, Re...) |
| [Intervals](intervals.md) | Step-distances and their frequency ratio equivalents |
| [Scales](scales.md) | Step-subsets: selecting pitches from the 12 available |
| [Chords](chords.md) | Step-sets and ratio-sets: simultaneous pitches |
| [Rhythm](rhythm.md) | Pulse-fraction system: T, onset, duration, cycle |
| [Missing Fundamental](missing-fundamental.md) | Virtual pitch: hearing frequencies that aren't there |

### Tier 2 — Essential Context

Deeper understanding, cross-cultural connections, and applied concepts.

| Page | Description |
|------|-------------|
| [Twelve-TET](twelve-tet.md) | The equal-temperament compromise and what it sacrifices |
| [Consonance & Dissonance](consonance-dissonance.md) | Roughness, harmonic template matching, cultural perception |
| [Pentatonic & Cross-Cultural](pentatonic-cross-cultural.md) | The universal 5-note scale and why it appears everywhere |
| [Translation Tables](translation-tables.md) | Complete PhizMusic ↔ Western ↔ other systems mapping |
| [Notation Layer](notation-layer.md) | Chromatic grid (Dodeka/piano roll) notation approach |
| [Chord Progressions](chord-progressions.md) | Step-set sequences, voice-leading, tension and resolution |
| [Timbre](timbre.md) | Spectral signatures, Sethares' timbre-tuning coupling |
| [Instrument Physics](instrument-physics.md) | String, pipe, and membrane acoustics |
| [ADSR Envelope](adsr-envelope.md) | Attack, decay, sustain, release — amplitude over time |
| [Digital Audio](digital-audio.md) | Sampling, Nyquist, aliasing, bit depth, room acoustics basics |
| [Auditory Masking](auditory-masking.md) | Frequency and temporal masking, MP3 compression connection |
| [Equal Loudness](equal-loudness.md) | Fletcher-Munson curves, frequency-dependent hearing sensitivity |

### Reference

| Page | Description |
|------|-------------|
| [Glossary](_glossary.md) | Canonical definitions of all PhizMusic terms |
| [Reference Table](_reference-table.md) | Complete step/syllable/Western/Hz/MIDI mapping |

### Future

| Page | Description |
|------|-------------|
| [ROADMAP](ROADMAP.md) | Phase 2 vision: interactive tooling, deferred content |

## Alphabetical Index

| Page | One-line description |
|------|---------------------|
| [ADSR Envelope](adsr-envelope.md) | Amplitude envelope: attack, decay, sustain, release phases |
| [Auditory Masking](auditory-masking.md) | How loud sounds render quiet sounds inaudible |
| [Chords](chords.md) | Simultaneous pitches described as step-sets and ratio-sets |
| [Chord Progressions](chord-progressions.md) | Sequences of step-sets with voice-leading analysis |
| [Consonance & Dissonance](consonance-dissonance.md) | Multi-factorial perception of tonal fusion vs. clash |
| [Digital Audio](digital-audio.md) | ADC, sampling rate, Nyquist theorem, aliasing, bit depth |
| [The Ear & Cochlea](ear-cochlea.md) | Biological frequency decomposition via basilar membrane |
| [Equal Loudness](equal-loudness.md) | Fletcher-Munson curves and frequency-dependent hearing |
| [Fourier Analysis](fourier-analysis.md) | Decomposing waveforms into component sine waves |
| [Frequency Ratios](frequency-ratios.md) | Pitch comparison by ratio, the cent measurement system |
| [Glossary](_glossary.md) | Canonical PhizMusic vocabulary definitions |
| [Harmonic Series](harmonic-series.md) | Integer-multiple overtone sequence from vibrating objects |
| [Instrument Physics](instrument-physics.md) | Acoustics of strings, pipes, and membranes |
| [Intervals](intervals.md) | Step-distances between pitches with ratio equivalents |
| [Missing Fundamental](missing-fundamental.md) | Perceiving pitch from harmonic pattern without the fundamental |
| [Naming System](naming-system.md) | Step-numbers (0-11) + octave, Dodeka syllables |
| [Notation Layer](notation-layer.md) | Chromatic grid notation based on Dodeka and piano roll |
| [Pentatonic & Cross-Cultural](pentatonic-cross-cultural.md) | The 5-note scale across world music traditions |
| [Reference Table](_reference-table.md) | Complete pitch mapping: step × octave → syllable → Hz |
| [Rhythm](rhythm.md) | Pulse-fraction system for temporal organization |
| [ROADMAP](ROADMAP.md) | Future plans: interactive tools, deferred content topics |
| [Scales](scales.md) | Step-subsets: selecting from the 12 chromatic pitches |
| [Timbre](timbre.md) | Spectral fingerprint of a sound, timbre-tuning coupling |
| [Translation Tables](translation-tables.md) | Bidirectional PhizMusic ↔ Western ↔ other systems |
| [Twelve-TET](twelve-tet.md) | Equal temperament: the engineering compromise |

## Mathematical Notation

Formulas are written in inline code blocks for portability:

- Frequency formula: `f = 440 × 2^((n - 69) / 12)` where n = MIDI note number
- Cent formula: `c = 1200 × log₂(f₂/f₁)`
- Harmonic series: `fₙ = n × f₁` for integer n

Future versions may render these using KaTeX for proper mathematical typesetting.

## The Dodeka Syllable System

The syllable system used throughout this wiki — **Do, Ka, Re, Xo, Mi, Fa, Hu, So, Bi, La, Ve, Si** — is adapted from the [Dodeka Music](https://dodekamusic.com/) system by Jacques-Daniel Rochat. Each of the 12 chromatic steps gets one unique syllable, with no sharps or flats needed. See [Naming System](naming-system.md) for full details.

## How to Contribute

Use the canonical template at [wiki/_template.md](_template.md) when creating or revising concept pages.

Contribution checklist:

- Include required front matter fields: `title`, `tier`, `scope-boundary` (plus aliases/tags/prerequisites/related)
- Keep one concept per page and add a `> 🎯 **Simple version**` block
- Add a `## Translation Table` and `## Connections` section where applicable
- Use PhizMusic-first vocabulary in explanatory text; keep Western terms in translation sections
- Add `<!-- INTERACTIVE: ... -->` placeholders only as future-facing notes (no embedded scripts in markdown)

## File Manifest

```
# Structural
wiki/README.md              — this file (index hub)
wiki/_template.md           — page template for contributors
wiki/_glossary.md           — canonical vocabulary reference
wiki/_reference-table.md    — complete pitch mapping table

# Tier 1 — Physics Primitives
wiki/sound-waves.md         — pressure waves, frequency, amplitude
wiki/ear-cochlea.md         — biological frequency analyzer
wiki/fourier-analysis.md    — waveform ↔ spectrum decomposition

# Tier 1 — Music Primitives
wiki/harmonic-series.md     — the overtone sequence
wiki/frequency-ratios.md    — pitch ratios and cents
wiki/naming-system.md       — step-numbers + Dodeka syllables
wiki/intervals.md           — step-distances and ratio equivalents
wiki/scales.md              — step-subsets
wiki/chords.md              — step-sets and ratio-sets
wiki/rhythm.md              — pulse-fraction temporal system
wiki/missing-fundamental.md — virtual pitch perception

# Tier 2 — Essential Context
wiki/twelve-tet.md              — equal temperament compromise
wiki/consonance-dissonance.md   — perceptual fusion mechanisms
wiki/pentatonic-cross-cultural.md — universal 5-note scale
wiki/translation-tables.md     — PhizMusic ↔ Western ↔ others
wiki/notation-layer.md         — chromatic grid notation
wiki/chord-progressions.md     — step-set sequences
wiki/timbre.md                 — spectral signatures
wiki/instrument-physics.md     — string, pipe, membrane acoustics
wiki/adsr-envelope.md          — amplitude envelope
wiki/digital-audio.md          — sampling, Nyquist, aliasing
wiki/auditory-masking.md       — frequency/temporal masking
wiki/equal-loudness.md         — Fletcher-Munson curves

# Future
wiki/ROADMAP.md             — Phase 2 interactive tooling vision
```
